...
"""Tests for the Kattis Interactive task type."""
...
from cms.grading.tasktypes.Kattis import Kattis
...
MGR_STATS_OK = { "exit_code": 42, **STATS_OK }
...
class TestEvaluate(TaskTypeTestMixin, FileSystemMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._mkdtemp_idx = 0
        patcher = patch("cms.grading.tasktypes.Kattis.tempfile.mkdtemp",
    ...
    def prepare(self, parameters, executables, managers):
        ...
        self.evaluation_step_after_run.return_value = (True, True, MGR_STATS_OK)
    ...
    def test_interactive(self):
        ...
        sandbox_mgr.create_file_from_storage.assert_has_calls([
            ...
            call("answer.txt", "digest of correct output")
        ], any_order=True)
        ...
        cmdline_mgr = ["./manager", "input.txt", "answer.txt", "/feedback"]
        cmdline_usr = ["run1", "foo", "foo"]
        self.evaluation_step_before_run.assert_has_calls([
            call(sandbox_mgr, cmdline_mgr, 4321, 1234 * 1024 * 1024,
                 dirs_map={
                     os.path.join(self.base_dir, "0"): ("/fifo", "rw"),
                     os.path.join(self.base_dir, "1"): ("/feedback", "rw")
                 },
                 stdin_redirect="/fifo/u_to_m",
                 stdout_redirect="/fifo/m_to_u",
                  multiprocess=True),
            call(sandbox_usr, cmdline_usr, 2.5, 123 * 1024 * 1024,
                 dirs_map={os.path.join(self.base_dir, "0"): ("/fifo", "rw")},
                 stdin_redirect="/fifo/m_to_u",
                 stdout_redirect="/fifo/u_to_m",
                 multiprocess=True),
        ], any_order=True)
        ...
        self.assertResultsInJob(job, True, str(1.0), ["Output is correct"], MGR_STATS_OK)
    ...
    def test_non_interactive(self):
        ...
        self.assertEqual(sandbox_usr.create_file_from_storage.call_count, 2)
        # Evaluation step called with the right arguments, in particular
        # redirects, and no (other) writable files. For the user's command,
        # see fake_evaluation_commands in the mixin.
        cmdline_mgr = ["./manager", "input.txt", "answer.txt", "/feedback"]
        ...
        self.evaluation_step_before_run.assert_has_calls([
            call(sandbox_usr, cmdline_usr, 2.5, 123 * 1024 * 1024,
                 dirs_map={os.path.join(self.base_dir, "0"): ("/output", "rw")},
                 stdin_redirect="input.txt",
                 stdout_redirect="/output/output.txt",
                 multiprocess=True),
            call(sandbox_mgr, cmdline_mgr, 4321, 1234 * 1024 * 1024,
                 dirs_map={
                     os.path.join(self.base_dir, "0"): ("/output", "rw"),
                     os.path.join(self.base_dir, "1"): ("/feedback", "rw")
                 },
                 stdin_redirect="/output/output.txt",
                  multiprocess=True),
        ], any_order=True)
        self.assertEqual(self.evaluation_step_before_run.call_count, 2)
        self.assertEqual(self.evaluation_step_after_run.call_count, 2)
